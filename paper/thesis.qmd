...
The remainder of the paper reviews the literature, sets out the theoretical framework and hypotheses, describes the data, develops the identification and empirical strategy, reports the main and mechanism results, and concludes.
...
## Weather, SAR, and per-crossing risk {#sec-weather-sar}
...
## Research question and hypotheses {#sec-hypotheses}
...
The main thesis statement is that the post-MoU interception regime should make the SWH--mortality slope less negative or more positive if it weakened the buffer between rough seas and death. The central prediction is therefore that the slope rises across the regime change. A second, mechanism-level prediction is that higher observed SAR activity should flatten the SWH--mortality slope. @sec-strategy formalizes this slope shift as the empirical estimand and sets out how it is identified and estimated.

The literature documents several behavioral responses to the post-2017 reconfiguration---shifts in crossing volume, boat composition, departure timing, and route choice. The channel this thesis examines, the *per-crossing risk* of dying given a crossing in given sea conditions, is distinct from these but cannot be separated from them without assumptions. The predicted rise in the slope corresponds to a movement in this channel.

If the slope shift is absorbed once exposure is controlled, the change is consistent with the volume and diversion channels alone. If it is absorbed once boat composition is controlled, the change is consistent with moral hazard alone. The prediction is that it persists under both; if it does, the per-crossing-risk channel has moved in addition to whatever else has changed.

# Data {#sec-data}

## Setting and sample

The analysis uses a daily panel for the Central Mediterranean Route, with weather, mortality, and crossing variables measured over the same area. Daily resolution is necessary because sea state changes quickly and can affect both departures and survival at sea. The design follows @deiana2024, which uses significant wave height (SWH) as an exogenous shock to crossing conditions on this route; monthly aggregation would absorb much of the identifying weather variation. The estimand, the change in the slope linking lagged SWH to recorded deaths across the post-MoU regime, is estimated on this panel by negative-binomial maximum likelihood and Poisson quasi-maximum likelihood with month--year fixed effects, the specification detailed below. Identification exploits day-to-day variation in sea state, which is plausibly exogenous to the migration-control regime: conditional on month--year fixed effects, the same maritime hazard recurs before and after the MoU, so a shift in how rough seas map into recorded deaths reflects a change in the regime that meets those seas, not a change in the seas themselves.
...
# Identification and Empirical Strategy {#sec-strategy}

## Estimand, decomposition, and identification {#sec-identification}
...
where $\beta_{\text{pre}}$ and $\beta_{\text{post}}$ are the slopes of log expected deaths on the lagged five-day mean of SWH before and after 2 February 2017, defined formally as the coefficients of the count regression in @sec-specification. For example, negative pre-period slopes are consistent with rough seas limiting departures when rescue availability is greater.
...
## Mechanism and robustness specifications {#sec-mechanism}
...
These results speak to three strands of work. The first links intensified or externalized border enforcement to higher migrant mortality --- at the United States land border [@cornelius2001; @solano2022], across the Mediterranean and Atlantic [@williams2018; @prieto-flores2025; @vives2023], and through cross-route diversion after the EU--Turkey Statement [@tafani2025]. That literature documents mortality rising largely because migrants are displaced onto more dangerous routes; the evidence here adds the complementary within-route margin, in which the same corridor's sea-state--mortality gradient steepens after the regime change rather than the risk simply relocating elsewhere. The second strand is the quantitative study of behavioral responses on the Central Mediterranean --- crossing volume [@rodriguezsanchez2023], boat composition and moral hazard [@deiana2024], and route choice [@hoffmannpham2024] --- which characterizes how many people cross and under what conditions. To these observable margins the analysis adds the per-crossing-risk margin they leave unobserved, and shows that the post-MoU slope shift persists after conditioning on the volume and composition channels those studies identify, so it is not reducible to them. The third treats search-and-rescue as a buffer that keeps the ordinary hazards of the crossing from escalating into death [@cusumano2020; @cusumano2023; @kosmas2022]; the mechanism estimates supply panel evidence consistent with that account, since weeks of higher recorded SAR activity flatten the weather--mortality gradient. Methodologically, the design repurposes the significant-wave-height variation that @deiana2024 use to explain crossing behavior in order to trace recorded mortality, and complements spatial evidence on where post-MoU incidents became deadlier [@zambiasi2025] with the time path of the sea-state gradient itself.
...
The implication is that sea-border policy should not be evaluated only through arrivals, interceptions, or aggregate deaths. The evidence is consistent with SAR acting as a safety buffer: when rescue is more available, rough seas translate less strongly into recorded mortality; when rescue is reduced or redirected, that buffer weakens. The policy therefore appears to increase the risk faced by the people who still depart, even when crossing volumes fall. The design measures this change in the risk gradient, not the number of lives that restoring rescue would save.
