...
#   output/figures/desc_panel_event_type.png  (detections + persons + composition)
...
stopifnot(all(c("date", "frx_persons", "frx_incidents", "iom_deaths",
                "crossing_attempts", "post_mou",
                "interceptions_daily", "arrivals") %in% names(panel)))
...
iom_cmr <- iom_raw %>%
  ...
  filter(!is.na(date), date <= FRX_END)
cat(sprintf("  IOM CMR: %d incidents\n", nrow(iom_cmr)))

# 1d. IOM monthly crossings
iom_monthly <- readRDS(file.path(BASE_DIR, "data", "processed",
  ...
  filter(ym <= floor_date(FRX_END, "month"))
...
monthly_panel <- panel %>%
  ...
  summarise(
    ...
    interceptions = sum(interceptions_daily),
...
# The daily panel uses crossing_attempts = Frontex + interceptions + deaths
...
p_cross_persons <- ggplot(crossing_persons, aes(x = ym, y = persons, fill = component)) +
  ...
  labs(title = "Number of persons attempting crossing in the Central Mediterranean, by crossing outcome",
       subtitle = "Monthly totals. Undetected arrivals computed as max(UNHCR daily arrivals in Italy - Frontex detections, 0) at monthly level.",
...
deaths_rate <- monthly %>%
  ...
  mutate(death_rate = ifelse(crossing_attempts > 0,
                              iom_deaths / crossing_attempts * 100, NA_real_))

scale_factor <- max(deaths_rate$iom_deaths, na.rm = TRUE) / 15

p_deaths <- ggplot(deaths_rate, aes(x = ym)) +
  geom_col(aes(y = iom_deaths, fill = "Deaths and missing persons (IOM)"), width = 25) +
  ...
  labs(title = "Number of dead and missing persons and fatality rate",
       subtitle = "Monthly totals. Fatality rate = deaths + missing / crossing attempts (Frontex + interceptions + deaths + missing).",
       x = NULL) +
...
etype_colours <- c(
  ...
  "LCG interceptions"          = "#252525",
  "TCG interceptions"          = "#969696"
)

# Frontex detections by full event type (counts)
frx_etype_n <- frx %>%
  mutate(ym = floor_date(date, "month"),
         etype = event_type) %>%
...
# 4a. Detections (events) by full event type
p_etype_n <- ggplot(frx_etype_n, aes(x = ym, y = n, fill = etype)) +
  geom_col(position = "stack", width = 25) +
  scale_fill_manual(values = etype_colours, drop = FALSE, name = NULL) +
  geom_vline(xintercept = MOU_DATE, linetype = "dashed", colour = "red", linewidth = 0.5) +
  coord_cartesian(xlim = PLOT_XLIM) +
  labs(title = "Frontex detections by event type",
       subtitle = "Monthly count of detection events by SAR and non-SAR categories.",
       y = "Detections", x = NULL) +
  etype_theme_noleg

# 4b. Persons by full event type (+ total crossing line)
crossings_monthly <- iom_monthly %>%
  left_join(
    iom_cmr %>%
      mutate(ym = floor_date(date, "month")) %>%
      group_by(ym) %>%
  ...
  mutate(total_crossings = replace_na(arrivals, 0) + deaths +
           replace_na(lcg, 0) + replace_na(tcg, 0)) %>%
  filter(ym <= FRX_END)
...
p_etype_p <- ggplot(persons_all, aes(x = ym, y = persons, fill = etype)) +
  ...
  scale_x_date(date_breaks = "1 year", date_labels = "%Y") +
  coord_cartesian(xlim = PLOT_XLIM) +
  labs(title = "Number of persons detected in the Central Mediterranean, by detection type",
       subtitle = "Monthly totals. Frontex persons by SAR/non-SAR category; LCG and TCG from IOM monthly data.",
       y = "Number of persons", x = NULL) +
  etype_theme_noleg
...
p_etype_pct <- ggplot(persons_smoothed, aes(x = ym, y = pct_smooth, fill = etype)) +
  ...
  scale_fill_manual(values = etype_colours, name = NULL) +
  scale_y_continuous(name = "Share of persons detected (%)") +
  geom_vline(xintercept = MOU_DATE, linetype = "dashed", colour = "red", linewidth = 0.5) +
  scale_x_date(date_breaks = "1 year", date_labels = "%Y") +
  coord_cartesian(xlim = PLOT_XLIM) +
  labs(title = "Composition of detection type, in percentage",
...
p_share_lines <- ggplot(share_lines, aes(x = ym, y = share, colour = group)) +
  ...
  labs(title = "Aggregate composition of detection type, in percentage",
       subtitle = paste0("SAR, non-SAR, LCG and TCG as share of total persons. LOESS-smoothed (span = ", LOESS_SPAN, ")."),
       y = "Share of persons detected (%)", x = NULL) +
  theme_minimal(base_size = 11) +
  theme(legend.position = "bottom",
        legend.text = element_text(size = 8),
        legend.margin = margin(t = -5))
...
p_boat_share <- ggplot(frx_boat, aes(x = ym, y = share, colour = boat)) +
  ...
  coord_cartesian(xlim = PLOT_XLIM, ylim = c(0, 100)) +
  scale_x_date(date_breaks = "1 year", date_labels = "%Y") +
  labs(title = "Share of detections in the Central Mediterranean, by boat type",
       subtitle = paste0("LOESS-smoothed (span = ", BOAT_SPAN, ")."),
       y = "Share of detection events (%)", x = NULL) +
  theme_minimal(base_size = 11) +
  theme(legend.position = "none")
...
iom_annual <- iom_cmr %>%
  ...
  summarise(
    iom_incidents = n(),
    iom_lethal = sum(dead_missing > 0),
    iom_deaths_total = sum(dead_missing),
...
# IOM monthly components (arrivals, LCG, TCG) by year
iom_comp_annual <- iom_monthly %>%
  mutate(year = year(ym)) %>%
  group_by(year) %>%
  summarise(arrivals = sum(arrivals, na.rm = TRUE),
            lcg = sum(lcg, na.rm = TRUE),
            tcg = sum(tcg, na.rm = TRUE),
            .groups = "drop")

# IOM all-CMR deaths by year (for total departures denominator)
iom_deaths_annual <- iom_cmr %>%
  mutate(year = year(date)) %>%
  group_by(year) %>%
  summarise(deaths_allcmr = sum(dead_missing), .groups = "drop")

# Merge
annual_summary <- panel_annual %>%
  left_join(iom_annual %>% select(year, iom_lethal, avg_deaths_per_lethal), by = "year") %>%
  left_join(iom_comp_annual, by = "year") %>%
  left_join(iom_deaths_annual, by = "year") %>%
  replace_na(list(deaths_allcmr = 0)) %>%
  mutate(total_crossings = arrivals + lcg + tcg + deaths_allcmr,
         arrival_share_pct = ifelse(total_crossings > 0,
...
var_labels <- c(
  ...
  frx_incidents         = "Frontex detections",
  iom_deaths            = "Deaths (IOM)",
  iom_lethal            = "Lethal events",
  death_rate_pct        = "Death rate (%)",
  avg_persons_event     = "Persons / event",
  avg_deaths_per_lethal = "Deaths / lethal event",
...
annual_transposed <- annual_summary %>%
  ...
  mutate(across(c(total_crossings, frx_incidents, iom_deaths, iom_lethal),
...
tbl <- annual_transposed %>%
  ...
  tab_source_note("Total crossing attempts = Frontex persons + LCG/TCG interceptions + deaths (IOM). Lower bound: excludes ~10% undetected arrivals.") %>%
  tab_source_note("Death rate = IOM deaths / crossing attempts.") %>%
  tab_source_note("Arrival share = Italian sea arrivals / total crossing attempts.")
