...
CMR_COUNTRIES <- c("Libya", "Tunisia", "Algeria", "Italy", "Malta")
LOESS_SPAN    <- 0.07
...
p <- ggplot() +
  ...
  geom_line(data = weekly,
            ...
            linewidth = 0.7) +
  geom_vline(xintercept = MOU_DATE,
             linetype = "dashed", colour = "grey30", linewidth = 0.4) +
  annotate("text", x = MOU_DATE, y = dr_max * 1.02,
...
ggsave(file.path(BASE_DIR, "output", "figures", "13_ts_deathrate_swh.png"),
       plot = p, width = 11, height = 5.5, dpi = 300)

cat("Saved: output/figures/13_ts_deathrate_swh.png\n")
