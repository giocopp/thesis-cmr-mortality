...
fit_and_report <- function(outcome, data, label) {
  ...
  f <- as.formula(paste0(outcome,
    " ~ swh_prevweek + swh_prevweek:post_mou | month_year"))

  m <- tryCatch(
    fenegbin(f, data = d, vcov = NW(14), panel.id = ~unit + date),
    error = function(e) { cat("    err:", conditionMessage(e), "\n"); NULL }
  )
  if (is.null(m)) return(invisible(NULL))

  ct <- coeftable(m, vcov = NW(14))
  b1_row <- which(rownames(ct) == "swh_prevweek")
...
cat("Spec: outcome ~ swh_prevweek + swh_prevweek:post_mou | month_year\n")
cat("NegBin (fenegbin), NW(14) SEs.\n\n")
