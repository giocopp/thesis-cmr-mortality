...
get_b3 <- function(m, vcov_type = NW(14)) {
...
m_da_base <- fenegbin(
  n_dead_missing ~ swh_prevweek + swh_prevweek:post_mou | month_year,
  data = d_da, vcov = NW(14), panel.id = ~unit + date
)
m_da_ctrl <- fenegbin(
...
ct_ctrl <- coeftable(m_da_ctrl, vcov = NW(14))
...
m_bl_base <- fenegbin(
  n_dead_missing ~ swh_prevweek + swh_prevweek:post_mou |
    month_year + sar_bloc,
  data = d_bl, vcov = NW(14), panel.id = ~sar_bloc + date
)
m_bl_ctrl <- fenegbin(
  n_dead_missing ~ swh_prevweek + swh_prevweek:post_mou +
...
ct_ctrl_bl <- coeftable(m_bl_ctrl, vcov = NW(14))
...
m_zp_base <- fenegbin(
  n_dead_missing ~ swh_prevweek + swh_prevweek:post_mou |
    month_year + country,
  data = d_zp, vcov = NW(14), panel.id = ~country + date
)
m_zp_ctrl <- fenegbin(
  n_dead_missing ~ swh_prevweek + swh_prevweek:post_mou +
...
ct_ctrl_zp <- coeftable(m_zp_ctrl, vcov = NW(14))
