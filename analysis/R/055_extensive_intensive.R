...
# =========================
# Enhancement #6: EXTENSIVE vs INTENSIVE MARGIN decomposition.
#
# Splits the "count of deaths" outcome into two separate mechanisms:
#   1) Extensive: P(any fatal incident on day t)
#        -> logit via fixest::feglm
#   2) Intensive: conditional count given a fatal incident (deaths > 0)
#        -> truncated NegBin via glmmTMB (family = truncated_nbinom2)
#
# Purpose: does weather shift the OCCURRENCE of fatal events, the SIZE of
# fatal events, both, or neither — and does that mechanism differ post-MoU?
#
# Truncated NegBin via glmmTMB (user-requested strict option). glmmTMB
# doesn't support fixest-style FE absorption, so month_year enters as a
# regular factor. Panel is small enough (~3-4k daily / ~16k zone rows) for
# this to be tractable.
#
# Flavors:
#   (A) daily-agg panel (daily_panel_complete.RDS)
#   (B) zone-panel (daily_panel_zone.RDS)
#
# Output: output/tables/055_extensive_intensive.txt
...
report_glmmtmb <- function(m, label, cluster_vec, n) {
  co <- fixef(m)$cond
  target <- "swh_prevweek_z:post_mou"
...
cat("055  EXTENSIVE vs INTENSIVE MARGIN\n")
cat("===================================\n")
cat("Target: swh_prevweek_z:post_mou coefficient\n\n")
...
for (ye in PERIODS) {
  ...
  # (1) Extensive: logit
  m_ext_da <- feglm(
    any_death ~ swh_prevweek_z + swh_prevweek_z:post_mou | month_year,
    data = d_da, family = binomial("logit"),
    vcov = NW(28), panel.id = ~unit + date
  )
  ct_ext_da <- coeftable(m_ext_da, vcov = NW(28))
  row_ext <- grep(":post_mou", rownames(ct_ext_da))

  cat(fmt_row("[A] extensive (logit, NW28)",
  ...
  m_int_da <- tryCatch(
    glmmTMB(n_dead_missing ~ swh_prevweek_z + swh_prevweek_z:post_mou +
              year_fac,
            data = d_da_int,
  ...
  m_ext_bl <- feglm(
    any_death ~ swh_prevweek_z + swh_prevweek_z:post_mou |
      month_year + sar_bloc,
    data = d_bl, family = binomial("logit"),
    vcov = NW(28), panel.id = ~sar_bloc + date
  )
  ct_ext_bl <- coeftable(m_ext_bl, vcov = NW(28))
  row_ext_bl <- grep(":post_mou", rownames(ct_ext_bl))

  cat(fmt_row("[B] extensive (logit, NW28)",
  ...
  ct_ext_zp <- coeftable(m_ext_zp, vcov = NW(28))
  row_ext_zp <- grep(":post_mou", rownames(ct_ext_zp))

  cat(fmt_row("[C] extensive (logit, NW28)",
