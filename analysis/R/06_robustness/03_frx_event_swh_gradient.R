# ── Frontex event-count SWH gradient (SAR vs Not-SAR) ─────────────────────────
...
sink_file <- tbl_path("06_robustness", "03_frx_event_swh_gradient.txt")
sink(sink_file)
...
# ── 5. LaTeX table (\input'd by paper/thesis.qmd) ─────────────
...
add("\\end{tabular}")
add("\\end{table}")
out_frx <- tbl_path("06_robustness", "03_frx_event_swh_gradient.tex")
writeLines(L, out_frx)
cat(sprintf("  Saved: %s\n", out_frx))
