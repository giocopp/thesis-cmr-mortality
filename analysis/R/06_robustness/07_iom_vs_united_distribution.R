...
fig_out <- fig_path("06_robustness", "07_iom_vs_united_distribution.png")
ggsave(fig_out, combined, width = 12, height = 8, dpi = 200)
cat(sprintf("\nSaved: %s\n", fig_out))

# ── 10. Save text output ─────────────────────────────────────
sink_file <- tbl_path("06_robustness", "07_iom_vs_united_distribution.txt")
sink(sink_file)
