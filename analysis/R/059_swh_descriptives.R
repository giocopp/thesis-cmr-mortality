...
# Out: output/tables/059_swh_descriptives.txt
#      output/figures/059_swh_heatmap.png      (year x decile share, 4 panels)
#      output/figures/059_swh_centre_of_mass.png  (annual centre of mass)
#      output/figures/059_pr_deadly_day.png    (annual extensive-margin)
...
d <- panel %>%
  filter(year(date) >= 2014, year(date) <= 2023, !is.na(swh_prevweek)) %>%
  mutate(
    yr          = year(date),
    deadly      = as.integer(n_dead_missing > 0),
    swh_decile  = ntile(swh_prevweek, 10)   # full-sample deciles
  )

# Decile boundaries (for the report)
decile_brk <- d %>%
  group_by(swh_decile) %>%
  summarise(swh_min = min(swh_prevweek),
            swh_max = max(swh_prevweek),
            swh_mid = round(median(swh_prevweek), 3),
            .groups = "drop")
...
centre_yr <- d %>%
  ...
  summarise(
    n_days                  = n(),
    `unweighted mean`       = mean(swh_prevweek),
    `deaths-weighted`       = wmean(swh_prevweek, n_dead_missing),
    `death-day SWH mean`    = if (sum(deadly) > 0) mean(swh_prevweek[deadly == 1]) else NA_real_,
    `frx-pers-weighted`     = wmean(swh_prevweek, frx_persons),
    `frx-inc-weighted`      = wmean(swh_prevweek, frx_incidents),
    .groups = "drop"
  )
...
cat("Saved: output/figures/059_swh_heatmap.png\n")
...
ggsave(file.path(BASE_DIR, "output", "figures",
                  "059_swh_centre_of_mass.png"),
       p_com, width = 10, height = 6, dpi = 200)
cat("Saved: output/figures/059_swh_centre_of_mass.png\n")
...
# Combine using patchwork if available, else save separately
if (requireNamespace("patchwork", quietly = TRUE)) {
  library(patchwork)
  p_combined <- p_pr / p_int
  ggsave(file.path(BASE_DIR, "output", "figures",
                    "059_pr_deadly_day.png"),
         p_combined, width = 10, height = 7, dpi = 200)
} else {
  ggsave(file.path(BASE_DIR, "output", "figures",
                    "059_pr_deadly_day.png"),
         p_pr, width = 10, height = 4, dpi = 200)
}
cat("Saved: output/figures/059_pr_deadly_day.png\n")

# ── 8. Save text output ────────────────────────────────────
sink_file <- file.path(BASE_DIR, "output", "tables", "059_swh_descriptives.txt")
sink(sink_file)
