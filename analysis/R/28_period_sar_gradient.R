...
ggsave(file.path(BASE_DIR, "output", "figures",
                  "28_period_sar_gradient.png"),
       p_all, width = 11, height = 14, dpi = 200)
cat("  Saved: output/figures/28_period_sar_gradient.png\n")

# ── 6. Save text output ──────────────────────────────────────
cat("\n--- 6. Saving results ---\n")

sink_file <- file.path(BASE_DIR, "output", "tables", "28_period_sar_gradient.txt")
sink(sink_file)
